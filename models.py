...
# Fare = IntegerField(default=0)
# migrate(migrator.add_column('Safari', 'Fare', Fare))
...
class Safari(BaseModel):
    ...
    @classmethod
    def create_safari(cls, bus_number, from_, Destination, Fare, depature_date, user):
        safari =cls.create(
...
class Payment(BaseModel):
    ...
    booking_id = ForeignKeyField(Booking, backref="bookingpay")
    ...
    @classmethod
    def create_payment(cls, transaction_id, booking_id, amount_paid, mobile_num):
...
def create_tables():
    with db:
        db.create_tables([User, Safari, Booking, Payment])
